//Как использовать данный скрипт и для чего он нужен:
//Скрипт используется для замера скорости работы функций в котировках.
//Для его запуска необходимо:
//	1) Cкомпилить код. 
//	2) Перейти на вкладку Backtesting.
//	3) Поставить параметры БТ:1m, 1 Day, Bid(+Ask), 1m-OHLC.
//	4) Выбрать первый чекбокс. 
//	5) Запустить бектестинг.После этого на диске D будает сформирван файл с называнием report.(Наличие идска D обязательно).
//	5) Выберите 2 чекбокс.Запускаем бектестинг и так далее.
//	
//	Не мало важно выбрирать чекбоксы по очереди и читать всплывающие подсказки.
...
extern bool T01_PrimitiveMath100000, T02_PrimitiveMath1000000, T03_PrimitiveMath10000000,T04_PredefinedVariables1000,
			T05_TimeseriesAccess50,T06_Concatenatio100,T07_TradeFunction=1,T08_ConversionFunctions50,T09_DateTimeFunctions100,
			...
			T16_AllIndicators,T17_MathTrig10000,T18_AllStringFunctions,T19_AllWindowFunctions,T20_ObjectFunctions;
...
void _03_PrimitiveMath10000000(){
	...
	for(int n=0; n<10; n++)
     {
		...
		int i=OrderSend(symb, OP_SELL, 0.01, Bid, 10, Ask+0.0005, Bid-0.0005, "I dont know why it's need", 123456, 0, White);
...
//Также тут должна быть проверка индикаторов, но она меняет тип файла.
...
string separators(){
	...
	if(T01_PrimitiveMath100000==1){ModuleSeparator=";";}
...
{	
    ...
    int ChackBoxSum=T01_PrimitiveMath100000+ T02_PrimitiveMath1000000+ T03_PrimitiveMath10000000+T04_PredefinedVariables1000+
...
int start()
{
    ...
    if (T01_PrimitiveMath100000==true) _01_PrimitiveMath100000();
    if (T02_PrimitiveMath1000000==true) _02_PrimitiveMath1000000();
    if (T03_PrimitiveMath10000000==true) _03_PrimitiveMath10000000();
    ...
    if (T17_MathTrig10000==true) _17_MathTrig10000();
    if (T18_AllStringFunctions==true) _18_AllStringFunctions();
    if (T19_AllWindowFunctions==true) _19_AllWindowFunctions();
    if (T20_ObjectFunctions==true) _20_ObjectFunctions();
  return(0);
}
...
{	
	...
	if(Quoute_counter>10000000){		//Вернуть на 100000 по окончании теста
		int answer2=MessageBox("Слишком много квот, работа будет прервана, меньший тайм фрейм или больший ренж", "Question", MB_OK|MB_ICONQUESTION);	
    	return(0);
		}
	...
	//---------------------------------------------------------------------------------------------
		...
		if (SizeFile==0){	//Если размер файла 0 то создаём хедер
			...
			int WriteFileHeader=FileWrite(file, CycleNumber,"\n;T01_PrimitiveMath100000;;;;;;;;T02_PrimitiveMath1000000;;;;;;;;T03_PrimitiveMath10000000;;;;;;;;",
			...
			"T14_ClientTerminal10000;;;;;;;;T14_ClientTerminal10000;;;;;;;;T15_CommonFunctions5;;;;;;;;T16_AllIndicators;;;;;;;;",
			...
			SecRowTemplate,SecRowTemplate,SecRowTemplate,SecRowTemplate,SecRowTemplate,SecRowTemplate);	//Записываем хедер таблицы

...
{	
	...
	if(sum>1){
//    	int FirstMessage=MessageBox("Вы выбрали более чем один пункт для отчёта, повторите попытку заново ", "Question", MB_OK|MB_ICONQUESTION);	//раскаментить как пофиксят
    	int FirstMessage=MessageBox("Вы выбрали более чем один пункт для отчёта, повторите попытку заново ", "Question");
    	One_Chose=false;
    	return(0);
    	}
	if(sum<1){
//    	int SecondMessage=MessageBox("Вы не выбрали не одного чекбокса, повторите попытку заново ", "Question", MB_OK|MB_ICONQUESTION);	//раскаментить как пофиксят
		int SecondMessage=MessageBox("Вы не выбрали не одного чекбокса, повторите попытку заново ", "Question");
    	One_Chose=false;
    	return(0);
    	}
    //открытие файла
...
//    	int answer1=MessageBox("Слишком мало квот, работа будет прервана, выберите больший тайм фрейм", "Question", MB_OK|MB_ICONQUESTION);	//раскаментить как пофиксят
		int answer1=MessageBox("Слишком мало квот, работа будет прервана, выберите больший тайм фрейм", "Question");
    	return(0);
		}
	if(Quoute_counter>100000){
//		int answer2=MessageBox("Слишком много квот, работа будет прервана, выберите больший тайм фрейм", "Question", MB_OK|MB_ICONQUESTION);	//раскаментить как пофиксят
		int answer2=MessageBox("Слишком много квот, работа будет прервана, выберите больший тайм фрейм", "Question");
    	return(0);
		}
